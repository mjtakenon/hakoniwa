...
use App\Entity\Cell\Monster\Boss;
...
class AppearanceBoss implements IDisaster
{
    ...
    public static function occur(Island $island, Terrain $terrain, Status $status, Turn $turn): DisasterResult
    {
        ...
        foreach ($candidates as $cell) {
            ...
            /** @var Boss $boss */
            $boss = new $monster(point: $cell->getPoint(), remain_move_times: 0);
            $logs->add(new AppearMonsterLog($island, $cell, $boss));

            $terrain->setCell($cell->getPoint(), $boss);
